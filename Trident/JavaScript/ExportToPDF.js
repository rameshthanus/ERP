...
function ExportToPDF(DivName,TableColumnHide,ReportHeading,PageType) {
    ...
    var printContents = '<html><head> <link href="../assets/admin/pages/css/invoice.css" media="print" rel="stylesheet" type="text/css" /><title></title><meta http-equiv="Content-Type" content="text/html; charset=utf-8" /></head><body><div style="font-size: 25px;" ><center>' + ReportHeading + '</center></div><br />' + DivName.html() + '</body></html>';

    $('body').prepend("<form method='post' action='../HttpHandler/ExportHandler.ashx' id='tempForm'><input type='hidden' name='data' value='" + printContents + "' ><input type='hidden' name='PageType' value='" + PageType + "' ></form>");
    $('#tempForm').submit();
    $("tempForm").remove(); 
    if (TableColumnHide) {
        $.each(TableColumnHide, function (index, value) {
