...
namespace Trident.HttpHandler
{
    ...
    /// </summary>
    public class SalesDelivery : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (mode == "get_dropdown" || mode == "get_autocode" || mode == "get_customerjob" || mode == "get_category" || mode == "get_GridPopup" || mode == "get_prod_details" || mode == "get_prod_update")
            {
                ...
                ob[0] = Convert.ToInt32(context.Request["SalesdeliveryID"]);
                ob[1] = Convert.ToInt32(context.Request["SalesdeliveryDetailID"]);
                ...
                ds = MSDataAccess.DataAccess.ExecuteDataset(Json.get_connectionstring(), "[spu_SalesDeliverySearch]", ob);
            ...
            else
            {
                ...
                else
                {
                    if (mode == "header")
                    {
                        object[] ob = new object[37];
                        ob[0] = context.Request["SalesdeliveryID"];
                        ...
                        ob[34] = context.Request["ShippingType"];
                        ob[35] = context.Request["ShippingAddressID"];
                        ob[36] = context.Request["CurrencyID"];
                        //
                        ds = MSDataAccess.DataAccess.ExecuteDataset(Json.get_connectionstring(), "[spu_SalesDeliveryHeader]", ob);
                        if (ds.Tables.Count > 0)
                        {
                    ...
                    else
                    {
                        ...
                        ob[0] = context.Request["SalesdeliveryDetailID"];
                        ob[1] = context.Request["SalesdeliveryID"];
                        ...
                        ds = MSDataAccess.DataAccess.ExecuteDataset(Json.get_connectionstring(), "[spu_SalesDeliveryDetail]", ob);
                        if (ds.Tables.Count > 0)
                        {
