...
{
    /// <summary>
    /// Summary description for AP_Payment
    /// </summary>
    public class AP_Payment : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (mode == "get_dropdown" || mode == "get_autocode" || mode == "get_customerjob" || mode == "get_category" || mode == "get_GridPopup" || mode == "get_prod_details" || mode == "get_prod_update")
            {
                ...
                ob[0] = Convert.ToInt32(context.Request["PaymentID"]);
                ob[1] = Convert.ToInt32(context.Request["Payment_DetailID"]);
                ...
                ds = MSDataAccess.DataAccess.ExecuteDataset(Json.get_connectionstring(), "[spu_ARPaymentSearch]", ob);
            ...
            else
            {
                ...
                else
                {
                    if (mode == "header")
                    {
                        ...
                        ob[0] = context.Request["PaymentID"];
                        ob[1] = context.Request["BranchID"];
                        ob[2] = context.Request["PaymentNo"];
                        ...
                        ob[5] = Convert.ToDateTime(funDatetime(context.Request["PaymentDate"]));
                        ...
                        ds = MSDataAccess.DataAccess.ExecuteDataset(Json.get_connectionstring(), "[spu_ARPaymentHeader]", ob);
                        if (ds.Tables.Count > 0)
                        {
                    ...
                    else
                    {
                        ...
                        ob[0] = context.Request["Payment_DetailID"];
                        ob[1] = context.Request["PaymentID"];
                        ...
                        ob[11] = context.Request["POID"];
                        ob[12] = context.Request["LineType"];
                        ob[13] = context.Request["Currency"];

                        ds = MSDataAccess.DataAccess.ExecuteDataset(Json.get_connectionstring(), "[spu_ARPaymentDetail]", ob);
                        if (ds.Tables.Count > 0)
                        {
