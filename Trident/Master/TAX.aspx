<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TAX.aspx.cs" Inherits="Trident.Master.TAX" %>
    ...
    <script type="text/javascript">
        ...
        var obj = {
            arg: function (scrn_name, imode, hmode) {
                Args = {
                    TAXID: get_TAXID(),
                    TAXCode: ($('#txt_code').val() == null ? '0' : $('#txt_code').val()),
                    TaxName: ($('#fv_txt_TAX').val() == '' ? '' : $('#fv_txt_TAX').val()),
                    TaxValue: ($('#fv_txt_TAXValue').val() == '' ? '0' : $('#fv_txt_TAXValue').val()),
                    TaxStatus: $("input[name=rad_status]:checked").val(),
                            ...
                            if (data.length > 0) {
                                if (sessionStorage.getItem('form_action') == "get_autocode") {
                                    ...
                                    if (sessionStorage.getItem('form_action') == "new_autocode") {
                                        ...
                                        if (sessionStorage.getItem('form_action') == "fetch_TAX") {
                                            $('#txt_code').val(data[0].TAXCode);
                                            $('#fv_txt_TAX').val(data[0].TaxName).removeClass('invalid').addClass('valid');
                                            $('#fv_txt_TAXValue').val(data[0].TaxValue).removeClass('invalid').addClass('valid');
                                            $("input[name=rad_status][value=" + data[0].TAXStatus + "]").prop('checked', true);
                                        } else {
                                            if (sessionStorage.getItem('form_action') == "update_TAX") {
                                                ...
                                                if (flag == 0) {
                                                    ...
                                                    if (data[0].design_count != undefined) {
                                                        ...
                                                        obj.autocode('TAX_Handler', 'TAX', 'Insert', 'update_autocode');
                                                    }
                                                }
                                                else {
                                                    if (flag == -1) {
                                                        ...
                                                        obj.autocode('TAX_Handler', 'TAX', 'AutoCode', 'get_autocode');
                                                    }
                                                }
                                            }
                                        }
            ...
            save: function () {
                var form_data = $("#" + "<%=frm_TAX.ClientID%>").serializeArray();
                var TAXformat = false;
                var TAX = $('#fv_txt_TAX').val();
                ...
                if (TAXformat) {
                    ...
                    if (!error_free) {
                        ...
                        if (sessionStorage.getItem('form_action') == "Insert_start") {
                            req += 1, flag = 0;
                            obj.autocode('TAX_Handler', 'TAX', 'Insert', 'save_TAX');
                        }
                        else {
                            sessionStorage.setItem('form_action', 'update_TAX');
                            obj.autocode('TAX_Handler', 'TAX', 'Update', 'update_TAX');
                        }
                    }
                }
                else {

                    $('#basic1').append(callalert('alert', 'Please check the GST.', '1'));
                    //$("#basic1").css("display", "block");
                    $('#basic1').modal('show');
                }
            },
            searchlist: function () {
                window.location.href = '../SearchList.aspx?pg=TAX';
            },
            Exist: function () {
                window.location.href = '../HomeContentPage.aspx';
            },
            clear: function () {
                $('#fv_txt_TAX,#fv_txt_TAXValue').val('').removeClass('valid').addClass('invalid');
                if (sessionStorage.getItem('form_action') != "get_autocode") {
        ...
        $(document).ready(function () {
            ...
            if (get_TAXID() == 0) {
                ...
                sessionStorage.setItem('form_action', 'fetch_TAX');
                obj.autocode('TAX_Handler', 'TAX', 'Fetch', 'fetch_TAX');
            }
...
<body>
    <form id="frm_TAX" runat="server">
        <div class="portlet box blue-hoki">
            <div class="portlet-title">
                <div class="caption">
                    <i class="fa fa-gift"></i>GST
                </div>
                <div class="tools">
                            ...
                            <table style="width: 100%">
                                ...
                                <tr>
                                    <td style="width: 100%">
                                        ...
                                        <div class="col-md-4">
                                            <input type="text" id="fv_txt_TAX" name="v_txt_TAX" class="invalid form-control" placeholder="eg:GST-7%" />
                                ...
                                <tr>
                                    <td style="width: 100%">
                                        <label class="col-md-3 control-label">
                                            GST Value<span class="required">
											* </span>
                                        </label>
                                        <div class="col-md-4">
                                            <input type="text" id="fv_txt_TAXValue" name="v_txt_TAXValue" class="invalid form-control" />
                                        </div>
                                    </td>
                                </tr>
                                <tr>
