<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Category.aspx.cs" Inherits="Trident.Master.Category1" %>
    ...
    <script type="text/javascript">
        ...
        var obj = {
            arg: function (scrn_name, imode, hmode) {
                Args = {
                    ...
                    CategoryCode: ($('#txt_code').val() == null ? '0' : $('#txt_code').val()),                   
                    CategoryName: ($('#fv_txt_category').val() == null ? '' : $('#fv_txt_category').val()),
                    CategoryStatus: $("input[name=rad_status]:checked").val(),
                            ...
                            if (data.length > 0) {
                                if (sessionStorage.getItem('form_action') == "get_autocode") {
                                    ...
                                    if (sessionStorage.getItem('form_action') == "new_autocode") {
                                        ...
                                        if (sessionStorage.getItem('form_action') == "fetch_category") {
                                            $('#txt_code').val(data[0].CategoryCode);                                           
                                            $('#fv_txt_category').val(data[0].CategoryName).removeClass('invalid').addClass('valid');
                                            $("input[name=rad_status][value=" + data[0].ColorStatus + "]").prop('checked', true);
                                        } else {
                                            if (sessionStorage.getItem('form_action') == "update_category") {
                                                ...
                                                if (flag == 0) {
                                                    req = 1; res = 0, flag = -1;
                                                    if (data[0].category_count != undefined) {
                                                        //alert('Item already exist');
                                                        ...
                                                        obj.autocode('category_autocode', 'Category', 'Insert', 'update_autocode');
                                                    }
                                                }
                                                else {
                                                    if (flag == -1) {
                                                        ...
                                                        obj.autocode('category_autocode', 'Category', 'AutoCode', 'get_autocode');
                                                    }
                                                }
                                            }
                                        }
            ...
            save: function () {
                var form_data = $("#" + "<%=frm_category.ClientID%>").serializeArray();
                var error_free = true;
                ...
                if (!error_free) {
                    ...
                    if (sessionStorage.getItem('form_action') == "Insert_start") {
                        req += 1, flag = 0;
                        obj.autocode('category_autocode', 'Category', 'Insert', 'save_category');
                    }
                    else {
                        sessionStorage.setItem('form_action', 'update_category');
                        obj.autocode('category_autocode', 'Category', 'Update', 'update_category');
                    }
                }
            },
            searchlist: function () {
                window.location.href = '../SearchList.aspx?pg=CATEGORY';
            },
            clear: function () {
                $('#fv_txt_category').val('').removeClass('valid').addClass('invalid');
        ...
        $(document).ready(function () {
            ...
            if (get_CategoryID() == 0) {
                ...
                sessionStorage.setItem('form_action', 'fetch_category');
                obj.autocode('category_autocode', 'Category', 'Fetch', 'fetch_category');
            }
...
<body>
    <form id="frm_category" runat="server">
            ...
            <div class="form-body">
                ...
                <td style="width:100%">
                 <label class="col-md-3 control-label">
                    ...
                    <div class="col-md-4">                
                        <input type="text" id="fv_txt_category" name="v_txt_category" class="invalid form-control" />
                    </div>
                </td>
                </tr>
                <tr>
