<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Event.aspx.cs" Inherits="Trident.CRM.Event" %>
...
<body>
    <form id="form1" runat="server">
        <div id="full_bdy">
            <span id="spn_lead_type">Creating New Event</span>
                                ...
                                <table class="table table-hover" id="tbl_listmain">
                                    <tbody>
                                        <tr>
                                            ...
                                            <td>
                                                <input type="text" class="form-control invalid" id="F_V_txt_Subject" name="V_txt_Subject" />
                                            </td>
                                            <td>
                                                <label>Visibility</label>
                                            </td>
                                            <td>
                                                <select class="form-control invalid" id="F_V_ddl_Visibility" name="ddl_Visibility">
                                                </select>
                                            </td>
                                        </tr>
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <div class="input-group input-medium date date-picker" data-date-format="dd-mm-yyyy" data-date-start-date="+0d">
                                                    <input type="text" class="form-control" id="F_V_txt_startdate" name="V_txt_startdate"/>
                                            ...
                                            <td>
                                                <%--  <input type="text" class="form-control invalid" id="F_V_txt_EndDate" name="txt_EndDate" />--%>
                                               <div class="input-group input-medium date date-picker" data-date-format="dd-mm-yyyy" data-date-start-date="+0d">
                                                    <input type="text" class="form-control" id="F_V_txt_enddate" name="V_txt_enddate"/>
                                                    <span class="input-group-btn">
                                                        <button class="btn default" type="button"><i class="fa fa-calendar"></i></button>
                                                    </span>
                                                </div>
                                                <div class="input-icon">
                                                    <i class="fa fa-clock-o"></i>
                                                    <input type="text" id="txt_endtime" class="form-control time" style="width: 120px;" />
                                                </div>
                                            </td>
                                        </tr>
                                        <tr>
                                            ...
                                            <td>
                                                <select class="form-control invalid" id="F_V_ddl_Status" name="V_ddl_Status">
                                                </select>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <input type="checkbox" id="F_V_txt_Notification" name="txt_Notification" />
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                            ...
                                            <td>
                                                <select class="form-control invalid" id="F_V_ddl_ActivityType" name="V_ddl_ActivityType">
                                                </select>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <input type="text" class="form-control invalid" id="F_V_txt_Location" name="txt_Location" />
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
    ...
    <script type="text/javascript">
        ...
        var obj_ = {
            arg: function (imode,id) {
                Args = {
                    Tittle: $('#F_V_txt_Subject').val(),
                    Type : 'Short',
                    Startdate: $('#F_V_txt_startdate').val(),
                    Starttime: $('#txt_starttime').val(),
                    Enddate: $('#F_V_txt_enddate').val(),
                    Endtime: $('#txt_endtime').val(),
                    StatusID: $('#F_V_ddl_Status').val(),
                    Notification: ($('#F_V_txt_Notification').prop('checked') == true ? 'T' : 'F'),
                    ActivityID: $('#F_V_ddl_ActivityType').val(),
                    Location: $('#F_V_txt_Location').val(),
                    PriorityID: $('#F_V_ddl_Priority').val(),
                    VisibilityID: $('#F_V_ddl_Visibility').val(),
                    RelatedTypeID: $('#ddl_relatetype').val(),
            ...
            Load_drpdowns: function (imode) {
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        //}
                        drpdowns = data;
                        $('#F_V_ddl_Status,#F_V_ddl_ActivityType,#F_V_ddl_Priority,#F_V_ddl_Visibility,#ddl_relatetype').empty();
                        if (data != null) {
                            if (data.length > 0) {
                                for (var i = 0; i < data.length; i++) {
                                    if (data[i].StatusID != null) {
                                        $('#F_V_ddl_Status').append($('<option></option>').val(data[i].StatusID).html(data[i].Status));
                                    } else if (data[i].ActivityTypeID != null) {
                                        $('#F_V_ddl_ActivityType').append($('<option></option>').val(data[i].ActivityTypeID).html(data[i].ActivityType));
                                    }
                                    else if (data[i].PriorityID != null) {
                                        $('#F_V_ddl_Priority').append($('<option></option>').val(data[i].PriorityID).html(data[i].Priority));
                                    }
                                    else if (data[i].VisibilityID != null) {
                                        $('#F_V_ddl_Visibility').append($('<option></option>').val(data[i].VisibilityID).html(data[i].Visibility));
                                    }
                                ...
                                var id = get_ID();
                                if (id != 0) {                                  
                                    $('#ddl_relatetype option[value=' + id.split('-')[1] + ']').attr('selected', 'selected');
                                    $('#ddl_relatetype').removeClass('invalid').addClass('valid');
                    ...
                    success: function (data) {
                        ...
                        if (data != null) {
                            if (data.length > 0) {
                                ...
                                var id = get_ID();
                                if (id != 0) {
                                    $('#ddl_relateto option[value=' + id.split('-')[2] + ']').attr('selected', 'selected');
                                    $('#ddl_relateto').removeClass('invalid').addClass('valid');
                                }
                            }
                        }
                    }
