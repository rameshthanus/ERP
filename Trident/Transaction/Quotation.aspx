<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Quotation.aspx.cs" Inherits="Trident.Transaction.Quotation" %>
...
<body onload="resizeIframe();">
                ...
                <div class="portlet-body cls_performo-header" style="height:100%;">
                    <div class="row">
                        <div class="col-sm-8 col-xs-12">
                            <div class="row">
                                <div class="col-xs-2">
                                    <label class="control-label req">Q.No.</label>
                                </div>
                                <div class="col-xs-4">
    ...
    <script type="text/javascript">
        ...
        var obj_ = {
            arg: function (screenname, imode, hmode, categoryid,id) {
                Args = {
                    QuotationID: id,
                    QuotationDetailID: 0,
            ...
            clear: function () {
                ...
                $('#F_V_ddl_jobno option[value=0],#F_V_ddl_Warehouse option[value=0], #F_V_ddl_Salesperson option[value=0],#F_V_ddl_customer option[value=0],#F_V_ddl_invoicetax option[value=0],#F_V_ddl_Currency option[value=0]').attr('selected', 'selected');
                ...
                obj_.get_autocode('Quotation', 'AutoCode', 'get_autocode');
            ...
            get_autocode: function (scname, imode, hmode) {
                $.ajax({
                    url: "../HttpHandler/Quotation.ashx",
                    ...
                    success: function (data) {
                        if (data != null) {
                            if (data.length > 0) {
                                ...
                                $('#F_V_txt_proformono').css('border', '1px solid red');
                            }
                        }
                    }
                });
            ...
            get_drpdowns: function (scname, imode, hmode) {
                $.ajax({
                    ...
                    success: function (data) {
                        debugger;
                        if (get_PerformoID() != 0) {
                            obj_.get_product_update(get_PerformoID());
                        }
                        ...
                        drpdowns = data;
                        $('#F_V_ddl_jobno,#F_V_ddl_customer,#F_V_ddl_invoicetax,#F_V_ddl_Warehouse,#ddl_category_filter,#F_V_ddl_Salesperson,#F_V_ddl_Currency').empty();
                        if (data != null) {
                            if (data.length > 0) {
            ...
            get_product_update: function (id) {
                $.ajax({
                    url: "../HttpHandler/Quotation.ashx",
                    data: this.arg(0, 'fetch', 'get_prod_update', 0, id),
                    cache: false,
                    success: function (data) {
                        main_data = [];
                        if (data != null) {
                            if (data.length > 0) {
                                for (var i = 0; i < data.length; i++) {
                                    if (data[i].N_QuotationNo != null) {
                                        debugger;
                                        sessionStorage.setItem('ID',get_PerformoID());
                                        $('#F_V_txt_proformono').val(data[i].N_QuotationNo).removeClass('invalid').addClass('valid');
                                        $("#F_V_txt_date").datepicker({ dateFormat: 'dd/mm/yyyy' }).datepicker("setDate", data[i].N_QuotationDate);                                        
            ...
            get_product_updateCopy: function (id) {
                $.ajax({
                    url: "../HttpHandler/Quotation.ashx",
                    data: this.arg(0, 'fetch', 'get_prod_update', 0, id),
                    cache: false,
            ...
            get_customerjob: function (scname, imode, hmode) {
                $.ajax({
                    url: "../HttpHandler/Quotation.ashx",
                    data: $.extend(this.arg(scname, imode, hmode, $('#F_V_ddl_shipping').val()), this.arg_('')),
                    cache: false,
                    success: function (data) {
                        if (data != null) {
                            if (data.length > 0) {                              
            ...
            get_shippercode: function (scname, imode, hmode) {
                $.ajax({
                    url: "../HttpHandler/Quotation.ashx",
                    data: $.extend(this.arg(scname, imode, hmode, $('#F_V_ddl_customer').val()), this.arg_('')),
                    async:false,
                    cache: false,
                    success: function (data) {
                        if (data != null) {
            ...
            save: function () {
                if (final_prod.length == 0) {
                    ...
                    if (!error_free) {
                        ...
                        debugger;
                        if (get_PerformoID() == 0 || sessionStorage.getItem('ID') == "0") {
                        ...
                        if (data != null) {
                            if (data.length > 0) {
                                ...
                                if (res == req) {
                                    ...
                                    if (get_PerformoID() == 0) {
                                        //obj_.get_autocode('Quotation', 'AutoCode', 'new_autocode');
                                    }
                                }
                            }
                        }
        ...
        $(document).ready(function () {
            ...
            obj_.get_drpdowns('Quotation', 'ComboBind', 'get_dropdown');
            obj_.load_popup('Quotation', 'GridPopup', 'get_GridPopup', 0);
            ...
            $(document).on('click', '#add_popup', function () {
                ...
                obj_.get_productdetails('Quotation', 'fetch_productdetail', 'get_prod_details', pop_performa_id);
                added_item = [];
            });
            ...
            $(document).on('click', '#Print_POP', function () {
                if (get_PerformoID() != 0 || sessionStorage.getItem('ID') != 0) {
                    ...
                    window.open("../Invoice/Invoice.aspx?ID=" + id + "&pg=QUOTATION", '_blank');
                }
            });
