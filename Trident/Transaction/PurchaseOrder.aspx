<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PurchaseOrder.aspx.cs" Inherits="Trident.Transaction.PurchaseOrder1" %>
...
<head runat="server">
  ...
  width: 350px;
        ...
        #sample3_wrapper .tabletools-dropdown-on-portlet {
            display: none;
        }
...
<body onload="resizeIframe();">
    <form runat="server" id="frm_so">
        <div id="full_bdy">
                ...
                <div class="portlet-body cls_performo-header" style="height: 246px;">
                    <div class="row">
                        <div class="col-sm-8 col-xs-12">
                            <div class="row">
                                <div class="col-xs-2">
                                    <label class="control-label req">PO No</label>
                                </div>
                                <div class="col-xs-4">
                                    <input type="text" class="form-control invalid" id="F_V_txt_pono" name="V_txt_pono" />
                                </div>
                                <div class="col-xs-2">
                            ...
                            <div class="portlet box blue-hoki">
                                ...
                                <div class="portlet-body" style="height: 185px;">
                                    ...
                                    <div class="col-xs-12 cls_category" style="padding: 0; display: none">
                                        ...
                                        <a class="btn purple-plum" id="btn_category_" data-toggle="modal" href="#" style="width: 103px; margin-top: 15px;display:none">Add </a>
                                    </div>
            ...
            <div class="row" id="dv_main_prod">
                <div class="col-xs-12">
                    ...
                    <div class="portlet box blue-hoki">
                        <div class="portlet-title">
                            <div class="caption">
                                <span id="rpt_caption">Purchase Order Detail</span>
                            </div>
                            <div class="tools">
                            </div>
                        </div>
            ...
            <div id="responsive1" class="modal fade" tabindex="-1" data-width="800">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
                    <h4 class="modal-title">History Track </h4>
                            ...
                            <div class="portlet box blue-hoki">
                                <div class="portlet-title">
                                    <div class="caption" style="width: 100%">
                                        <span id="rpt_caption_pop1" style="float: left">Products</span>
                                        <div style="float: right; position: relative; /* left: 538px; */ z-index: 3">
                                            <%--    <button type="button" data-dismiss="modal" class="btn btn-default">Close</button>--%>
                                            <button type="button" class="btn blue" id="confirm_h">Confirm</button>
                                        </div>
                                    </div>
                                    <div class="tools">
                                    </div>
                                ...
                                <div class="portlet-body">
                                    <table class="table table-striped table-bordered table-hover" id="sample3">
                                        <thead>
                                            <tr>
                                            </tr>
                                        </thead>
    ...
    <script type="text/javascript">
        ...
        var drpdowns = [], req, old_data = 0, res, obj_tbl, pop_performa_id = '', obj_tbl_main, added_item = [], final_prod = [], final_prod_code = [], popup_data = [];
        ...
        var initTable5 = function (tbl) {
            ...
            var oTable = table.dataTable({
                ...
                // $('#' + tbl + '_wrapper table thead tr th:nth-child(1)').trigger('click');
                //  },
        ...
        var obj_ = {
            arg: function (screenname, imode, hmode, categoryid) {
                Args = {
                    ...
                    IndentID: ($('#F_V_ddl_indentno').val() == null ? 0 : $('#F_V_ddl_indentno').val()),
            ...
            arg_header: function (total_qty, imode, hmode) {
                Arg_h = {
                    PurchaseOrderID: get_POID(),
                    BranchID: get_BranchID(),
                    PurchaseOrderNo: ($('#F_V_txt_pono').val() == "" ? 0 : $('#F_V_txt_pono').val()),
                    PurchaseOrderDate: ($('#F_V_txt_date').val() == "" ? '1990/01/01' : $('#F_V_txt_date').val()),
            ...
            arg_detail: function (prod_Detail, ProdID, index, hmode, imode) {
                Arg_d = {
                    ProductDetailID: prod_Detail,
                    PurchaseOrderID: ProdID,
                    ProductCode: $('#Code_' + index).text(),
                    Description: $('#Description_' + index).val(),
                    Qty: $('#Qty_' + index).val(),
                    Rate: $('#Rate_' + index).val(),
                    Amount: $('#Amount_' + index).text(),
                    PartyID: $('#Supplier_' + index + ' select').val(),
                    DeliveryDate: $('#DeliveryDate_' + index).val(),
                    imode: imode,
                    hmode: hmode
                }
                return Arg_d;
            ...
            searchlist: function () {
                window.location.href = '../SearchListTransaction.aspx?pg=PURCHASEORDER';
            },
            clear: function () {
            ...
            addnew_: function () {
                ...
                obj_.clear();
                obj_.get_autocode('PurchaseOrder', 'new-AutoCode', 'new_autocode');
            },
            exit: function () {
            ...
            get_autocode: function (scname, imode, hmode) {
                $.ajax({
                    url: "../HttpHandler/PurchaseOrder.ashx",
                    data: $.extend(this.arg(scname, imode, hmode, 0), this.arg_('')),
                    cache: false,
                    success: function (data) {
                        if (data != null) {
                            if (data.length > 0) {
                                $('#F_V_txt_pono').val(data[0].Column1);
                                $('#F_V_txt_pono').removeClass('invalid').addClass('valid');
                                $('#F_V_txt_pono').css('border', '1px solid red');
                            }
                        }
                    }
                });
            ...
            get_drpdowns: function (scname, imode, hmode) {
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        drpdowns = data;
                        $('#F_V_ddl_jobno,#F_V_ddl_indentno,#ddl_category_filter').empty();
                        if (data != null) {
                            if (data.length > 0) {
                                for (var i = 0; i < data.length; i++) {
                                    if (data[i].PartyName != null) {
                                        // $('#F_V_ddl_supplier').append($('<option></option>').val(data[i].PartyID).html(data[i].PartyName));
                                        sid.push(data[i].PartyID);
                                        snm.push(data[i].PartyName);
                                    } else
                                        if (data[i].JOBCode != null) {
                                            $('#F_V_ddl_jobno').append($('<option></option>').val(data[i].JOBID).html(data[i].JOBCode));
                                        }
                                        else if (data[i].IndentID != null) {
                                            $('#F_V_ddl_indentno').append($('<option></option>').val(data[i].IndentID).html(data[i].IndentNo));
                                        }
                        ...
                        for (var i = 0; i < sid.length; i++) {
                            sp_ += '<option value=' + sid[i] + '>' + snm[i] + '</option>';
                        }
                        sp_ = '<select class="cls_main_sp">' + sp_ + '</select>';
                    }
                });
            },
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data != null) {
                            if (data.length > 0) {
                                for (var i = 0; i < data.length; i++) {
                                    if (data[i].N_PurchaseOrderNo != null) {
                                        $('#F_V_txt_pono').val(data[i].N_PurchaseOrderNo).removeClass('invalid').addClass('valid');
                                        $('#F_V_txt_date').val(data[i].N_PurchaseOrderDate).removeClass('invalid').addClass('valid');
                                        ...
                                        $('#F_V_ddl_indentno option[value=' + data[i].N_IndentID + ']').attr('selected', 'selected');
            ...
            config_maintable: function (data, mode) {
                            ...
                            if (key.indexOf('Qty') > -1) {
                                ...
                                if (key.indexOf('Rate') > -1) {
                                    ...
                                    if (key.indexOf('Supplier') > -1) {
                                        str += '<td><div id="' + $('#sample_1 thead tr th:nth-child(' + j + ')').text() + '_' + i + '">' + sp_ + '</div></td>';
                                    } else {
                                        if (key.indexOf('Description') > -1) {
                                            str += '<td> <textarea autofocus class="form-control cls_desc" style="width:300px;" id="' + $('#sample_1 thead tr th:nth-child(' + j + ')').text() + '_' + i + '" rows="3"> ' + value + '</textarea></td>';
                                        } else {
                                            if (key.indexOf('DeliveryDate') > -1) {
                                                str += '<td><input type="text" class="cls_deliverydate date-picker" style="width:90px" id="' + $('#sample_1 thead tr th:nth-child(' + j + ')').text() + '_' + i + '"></input></td>';
                                            } else {
                                                if (key.indexOf('Code') > -1) {
                                                    str += '<td><span id="' + $('#sample_1 thead tr th:nth-child(' + j + ')').text() + '_' + i + '"><a href="#" onclick=h_("' + value + '","' + i + '")>' + value + '</a></span></td>';
                                                } else {
                ...
                for (var i = 0; i < final_prod.length + removed_data.length; i++) {
                    if (removed_data.indexOf(i.toString()) == -1) {
                        ...
                        $('#DeliveryDate_' + i).val(all_data[i].DeliveryDate).addClass('valid').removeClass('invalid');
                        $('#Supplier_' + i + ' select option[value=' + all_data[i].Supplier + ']').attr('selected', 'selected').addClass('valid').removeClass('invalid');
                    }
                }
                $('#responsive').modal('hide');
                if (mode == '1' || mode == '3') {
            ...
            save: function () {
                if (final_prod.length == 0) {
                    ...
                    if (!error_free) {
                        ...
                        if (get_POID() == 0 || sessionStorage.getItem('mode')=='new') {
            ...
            save_header: function (imode, hmode) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data != null) {
                            if (data.length > 0) {
                                if (data[0].PONo_count == 0) {
                                    SalesId = data[0].PurchaseOrderID;
                                    res = 0;
                                    if (get_POID() == 0 || sessionStorage.getItem('mode') == 'new') {
                                        ...
                                        for (var j = 0; j < all_data.length; j++) {
                                            if (removed_data.indexOf(j.toString()) == -1) {
                                                obj_.save_detail(0, get_POID(), j, 'details', 'insert');
                                            }
                                        }
                                    }
                                } else {
        ...
        h_ = function (code_, ind) {
            $('#responsive1').modal('show');
            //$('#confirm_h').css('display', 'block');
            //setTimeout(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data != null) {
                        if (obj_tbl3 != undefined) {
                            $('#sample3').dataTable().fnDestroy();
                        }
                        $('#sample3 thead tr').empty();
                        $('#sample3 tbody').empty();
                        if (data.length > 0) {
                            var sth = '<th></th>';
                            $.each(data[0], function (key, value) {
                            ...
                            var i = 0;
                            $.each(data, function (key, value) {
                                str += '<tr><td><span id="' + ind + '"><input type="checkbox" class="chk_" id="chk_' + i + '"/></span></td>';
                                $.each(value, function (key, value) {
                                    //if (key.indexOf('N_') > -1) {
                            ...
                            $('#sample3 thead tr').append(sth);
                            $('#sample3 tbody').append(str);
                        } else {
                            $('#sample3 thead tr').append('<th></th>');
                            $('#sample3 tbody').append('<tr><td>No Data</td></tr>');
                            // $('#confirm_h').css('display', 'none');
                        }
                        obj_tbl3 = initTable5('sample3');
                        $("#sample3 thead tr th.hde_h").filter(function (index) {
                            // console.log($(this).index());
                            // console.log(index);
                            //$('#sample3 thead tr th[class="hde_"]').filter(function (index) {
                            // alert()
                            $('#sample3 tbody tr td:nth-child(' + ($(this).index() + 1) + ')').addClass('hde_h');
                        });
                        checkForChanges();
                    }
                }
        ...
        function checkForChanges() {
            // alert();
            if ($('#responsive1').hasClass('in'))
                $('#sample3_wrapper table thead tr th:nth-child(1)').trigger('click');
            else
        ...
        $(document).ready(function () {
            ...
            if (get_POID() == 0) {
                obj_.get_autocode('PurchaseOrder', 'AutoCode', 'get_autocode');
            }
            obj_.get_drpdowns('PurchaseOrder', 'ComboBind', 'get_dropdown');
            obj_.load_popup('PurchaseOrder', 'GridPopup', 'get_GridPopup', 0);
            ...
            $(document).on('click', '#confirm_h', function () {
                if ($('.chk_').is(':checked')) {
                    var ind = $('.chk_:checked')[0].id.split('_')[1];

                    $('#Supplier_' + $('.chk_:checked').parent()[0].id + ' select option[value=' + $('#N_PartyID_' + ind).text() + ']').attr('selected', 'selected');
                }
            ...
            $(document).on('change', '#F_V_ddl_jobno', function () {
                if ($(this).val() != 0) {
                    obj_.get_customerjob('PurchaseOrder', 'GetCustomerJOB', 'get_customerjob');
                } else {
            ...
            $(document).on('click', '#add_popup', function () {
                ...
                obj_.get_productdetails('PurchaseOrder', 'fetch_productdetail', 'get_prod_details', pop_performa_id);
                added_item = [];
            });
            ...
            $(document).on('change', '.cls_main_sp', function () {
                all_data[$(this).parent()[0].id.split('_')[1]].Supplier = $(this).val();
            });
            $(document).on('change', '.cls_deliverydate', function () {
                all_data[$(this)[0].id.split('_')[1]].DeliveryDate = $(this).val();
            });
